---
redirectFrom:
  - "/ai-ml/optimization/particle-swarm-optimization/constriction_factor_particle_swarm_optimization.html"
title: "收缩因子粒子群优化（CFPSO）"
date: 2026-03-21
order: 5
category:
  - 智能优化算法
...
- 个体最优位置（`pbest`）：每个粒子的个体最优位置。
- 全局最优位置（`gbest`）：粒子群当前找到的全局最优位置。
...
1. 初始化粒子群：
    1. 对粒子群中的每个粒子：
        1. 在搜索空间边界内随机初始化粒子位置。
        2. 将粒子速度初始化为 0 或较小的随机值。
        3. 利用目标函数评估粒子的适应度。
        4. 将粒子的初始位置设为其个体最优位置 `pbest`。
    2. 将粒子群中最优粒子的位置设为全局最优位置 `gbest`。
2. 当终止条件未满足时，重复执行：
    1. 对粒子群中的每个粒子：
        1. 使用收缩因子公式更新粒子速度：
      ...
      - `x_new = x_old + v_new`
        3. 利用目标函数评估粒子适应度。
        4. 若粒子新位置的适应度优于其 `pbest` 对应的适应度：
      - 将粒子的 `pbest` 更新为新位置。
        5. 若粒子新位置的适应度优于粒子群当前 `gbest` 的适应度：
      - 将 `gbest` 更新为该粒子的新位置。
3. 返回 `gbest` 作为最终找到的最优解。
...
- 收缩因子 `chi` 通常可设置在 0.6 到 0.8 之间，常用取值为 `chi = 0.729`。
- 认知分量权重 `phi1` 和社会分量权重 `phi2` 通常满足 `phi1 + phi2 > 4`，常见设置为 `phi1 = phi2 = 2.05`。
- 应根据问题复杂度调整粒子群规模 `num_particles`。通常可采用 20 到 50 个粒子；对于高维问题，可能需要更大的种群规模。
...
- 根据问题复杂度与可用计算资源设置最大迭代次数 `num_iterations`。
- 若已知最优解适应度值或能够进行合理估计，可采用适应度阈值作为终止条件。
